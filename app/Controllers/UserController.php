...
class UserController extends Controller
{
    ...
    public function generateReport($id)
    {
        ...
        $roles = [1 => 'Admin', 2 => 'Artist', 3 => 'Listener', 4 => 'Producer'];
        $roleName = $roles[$user->role_id] ?? 'Unknown';
        $pdf->Cell(0, 8, "Role: {$roleName}", 0, 1);
        ...
        if ($user->role_id == 2)
        {
            ...
            $artist = Artist::getByUserId($user->id);
            if ($artist)
            {
                $pdf->Ln(5);
                $pdf->SetFont('Arial', 'B', 14);
                ...
                $albums = Album::getByArtistId($artist->id);
                if ($albums)
                {
                    $pdf->Ln(3);
        ...
        if ($user->role_id == 4)
        {
            ...
            if ($producer)
            {
                ...
                $pdf->SetFont('Arial', '', 12);
                $pdf->MultiCell(0, 8, "Studio Name: {$producer->studio_name}", 0, 1);
                $pdf->MultiCell(0, 8, "Bio: {$producer->bio}");

                // list productions
                $productions = ProducerAlbum::getAllByProducerWithNames($producer->id);
                if (!empty($productions))
                {
                    $pdf->Ln(3);
                    $pdf->SetFont('Arial', 'B', 12);
                    $pdf->Cell(0, 8, "Productions:", 0, 1);
                    $pdf->SetFont('Arial', '', 11);
                    foreach ($productions as $prod)
                    {
                        $pdf->Cell(0, 6, "- {$prod['album_title']} (Artist: {$prod['artist_name']})", 0, 1);
                    }
                }
                else
                {
                    $pdf->Ln(2);
                    $pdf->Cell(0, 6, "No productions yet.", 0, 1);
                }
            }
        }
